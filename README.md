...
```sh
uv run python main.py --model-path ~\.ai_qwen\models\qwen2.5-3b\
```

启动 OpenAI-compatible 服务（对外暴露接口，内部走本地权重）

```shell
uv run python openai_compatible_api.py --model-path ~\.ai_qwen\models\qwen2.5-3b --served-model-name qwen-local --host 0.0.0.0 --port 8000
```
